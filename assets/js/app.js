...
let view = (typeof location !== "undefined" && location.hash.replace("#","")) || "overview";
/* Modules (grid.js, cap.js) register extra views before this file runs. */
const EXT = (typeof window !== "undefined" && window.BOARD_EXT) || {views:[],render:{},wire:[]};
...
// Each registered view is [key, title, subtitle, insertAfterKey].
(EXT.views || []).forEach(v => {
  const at = VIEWS.findIndex(x => x[0] === v[3]);
  VIEWS.splice(at === -1 ? VIEWS.length : at + 1, 0, [v[0], v[1], v[2]]);
});

function setHash(){
  if(typeof history === "undefined") return;
  try{ history.replaceState(null, "", "#" + view); }catch(e){/* file:// blocks this */}
}

/* Following a link to #cap while the board is already open changes only the
   fragment, so the browser fires no reload. Without this the shared link
   appears to do nothing. */
if(typeof window !== "undefined"){
  window.addEventListener("hashchange", () => {
    const v = location.hash.replace("#", "");
    if(v && v !== view && VIEWS.some(x => x[0] === v)){
...
function render(){
  ...
  document.getElementById("nav").querySelectorAll("button").forEach(b=>
    b.onclick=()=>{view=b.dataset.v;setHash();window.scrollTo(0,0);render()});
  document.getElementById("footMeta").innerHTML=
    `<span class="mono">${S.meta.standard} · ${S.meta.issued} · ${S.items.length} clauses · `
    + `${S.tasks.length} tasks · ${(S.caps||[]).length} CAPs</span>`;
  document.getElementById("view").innerHTML = Object.assign({
    overview:vOverview, checklist:vChecklist, tasks:vTasks, orsvai:vOrsvai,
    plan:vPlan, performance:vPerf, register:vRegister
  }, EXT.render)[view]();
  wire();
  (EXT.wire || []).forEach(f => { try{ f(); }catch(err){ console.error("module wire failed", err); } });
}
...
function vOverview(){
  ...
  ${typeof capMetrics === "function" && (S.caps||[]).length ? capBand() : ""}
...
/* Corrective action progress, summarised for the Overview. */
function capBand(){
  const c = capMetrics();
  return `
  <div style="height:22px"></div>
  <div class="card">
    <div class="hd"><h3>Corrective action plan</h3>
      <span class="note">${c.total} CAPs raised · ${c.closed} closed and verified</span>
      <div class="spacer"></div>
      <button class="btn sm" id="goCap">Open the CAP register</button></div>
    <div class="bd">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px">
        <b style="font-size:13px">CAP completion — severity weighted</b>
        <span class="mono" style="font-size:12px;color:var(--mute)">${c.completion.toFixed(1)}%</span>
      </div>
      <div class="bar ${c.completion>=100?"done":""}" style="height:8px"><span style="width:${c.completion}%"></span></div>
      <div class="kpis" style="margin-top:16px">
        <div class="kpi ${c.zeroTol?"bad":"good"}"><div class="eyebrow">Zero-tolerance CAPs open</div>
          <div class="v">${c.zeroTol}</div><div class="d">Cat 5 and Cat 6 findings still running</div></div>
        <div class="kpi ${c.overdue?"bad":""}"><div class="eyebrow">Overdue</div>
          <div class="v">${c.overdue}</div><div class="d">Past target or revised target</div></div>
        <div class="kpi ${c.awaiting?"warn":""}"><div class="eyebrow">Awaiting verification</div>
          <div class="v">${c.awaiting}</div><div class="d">Done, evidence not yet checked</div></div>
        <div class="kpi ${c.noPreventive?"warn":""}"><div class="eyebrow">Without preventive action</div>
          <div class="v">${c.noPreventive}</div><div class="d">Fixes the instance, not the recurrence</div></div>
        <div class="kpi ${c.ineffective?"bad":""}"><div class="eyebrow">Effectiveness reviewed</div>
          <div class="v">${c.assessed?`${c.effective}/${c.assessed}`:"—"}</div>
          <div class="d">${c.assessed?`${c.ineffective} did not hold`:"No closure reviewed yet"}</div></div>
      </div>
    </div>
  </div>`;
}
...
function wire(){
  ...
  const gc=document.getElementById("goCap");
  if(gc) gc.onclick=()=>{view="cap";setHash();window.scrollTo(0,0);render()};
...
document.getElementById("fileIn").onchange=e=>{
  ...
  r.onload=()=>{try{const j=JSON.parse(r.result);
      ...
      S=j; if(!S.tasks)S.tasks=[]; if(!S.caps)S.caps=[]; if(!S.projectStart)S.projectStart=todayISO();
      render(); toast("Board loaded — "+S.items.length+" clauses, "+S.tasks.length
        +" tasks, "+S.caps.length+" CAPs");
...
/* Modules reach the board through this handle rather than closing over `S`,
   which is reassigned wholesale on import. */
window.BOARD = {
  get state(){ return S; },
  get view(){ return view; },
  go(v){ view = v; window.scrollTo(0, 0); render(); },
  render,
};
